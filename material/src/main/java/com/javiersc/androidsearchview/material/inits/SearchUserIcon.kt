...
internal fun <T> MaterialSearchView<T>.initSearchUserIcon(attrs: TypedArray) {
    val materialSearchView: MaterialSearchView<T> = this
    with(attrs) {
        val searchUserIconEnabled: Boolean = getBoolean(
        ...
        if (searchUserIcon != 0) materialSearchView.context.drawable(searchUserIcon)?.let { icon ->
            materialSearchView.searchUserIcon = icon
        }
        else materialSearchView.searchUserIcon = materialSearchView.searchUserIcon

        val searchUserIconColor: Int = getColor(
        ...
        val searchUserIconBorderColor: Int = getColor(
        ...
        val searchUserIconBorderWidth: Float = getDimension(
        ...
        imageButtonUser.setOnClickListener {
            materialSearchView.onSearchUserIconListener?.invoke()
        }
    }
}