...
internal fun <T> MaterialSearchView<T>.initSearchUpIcon(attrs: TypedArray) {
    ...
    with(attrs) {
        ...
        if (searchUpIcon != 0) materialSearchView.context.drawable(searchUpIcon)?.let {
            materialSearchView.searchUpIcon = it
        }
        else materialSearchView.searchUpIcon = materialSearchView.searchUpIcon

        val searchUpIconColor: Int = getColor(
        ...
        val searchUpIconDuration: Long = getInteger(
        ...
        val searchUpIconMarginLeft: Float = getDimension(
