...
internal fun <T> MaterialSearchView<T>.initSearchClearIcon(attrs: TypedArray) {
    val materialSearchView: MaterialSearchView<T> = this
    with(attrs) {
        val searchClearIconEnabled: Boolean = getBoolean(
        ...
        if (searchClearIcon != 0) materialSearchView.context.drawable(searchClearIcon)?.let {
            materialSearchView.searchClearIcon = it
        }
        else materialSearchView.searchClearIcon = materialSearchView.searchClearIcon

        val searchClearIconColor: Int = getColor(
