...
internal fun <T> MaterialSearchView<T>.initSearchMicIcon(attrs: TypedArray) {
    val materialSearchView: MaterialSearchView<T> = this
    with(attrs) {
        val searchMicIconEnabled: Boolean = getBoolean(
        ...
        if (searchMicIcon != 0) materialSearchView.context.drawable(searchMicIcon)?.let { icon ->
            materialSearchView.searchMicIcon = icon
        }
        else materialSearchView.searchMicIcon = materialSearchView.searchMicIcon

        val searchMicIconColor: Int = getColor(
