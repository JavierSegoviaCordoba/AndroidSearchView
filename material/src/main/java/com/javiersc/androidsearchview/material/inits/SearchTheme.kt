...
internal fun <T> MaterialSearchView<T>.initSearchTheme(attrs: TypedArray) {
    val materialSearchView: MaterialSearchView<T> = this
    with(attrs) {
        val searchTheme: Int = getInteger(
            R.styleable.MaterialSearchView_searchTheme,
            materialSearchView.searchTheme.value
        )
        materialSearchView.searchTheme = SearchTheme.values()
            .find { theme: SearchTheme -> theme.value == searchTheme }
            ?: SearchTheme.LIGHT
    }
}