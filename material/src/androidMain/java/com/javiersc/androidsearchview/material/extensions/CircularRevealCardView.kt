...
fun CircularRevealCardView.show(
    ...
    this.post {
        ...
        val startRadius =
            if (!isBackground)
                hypot(this.measuredWidth.toDouble(), this.measuredHeight.toDouble()).toFloat()
            else hypot(screenSize.x.toDouble(), screenSize.y.toDouble()).toFloat()

        if (this.isAttachedToWindow) {
            val animator =
                CircularRevealCompat.createCircularReveal(this, centerX, centerY, 0f, startRadius)
                    .apply {
...
fun CircularRevealCardView.hide(
    ...
    this.post {
        ...
        if (this.isAttachedToWindow) {
            val animator =
                CircularRevealCompat.createCircularReveal(this, centerX, centerY, startRadius, 0f)
                    .apply {
                        this.duration = duration
                        this.interpolator = interpolator
                        startDelay = delay
