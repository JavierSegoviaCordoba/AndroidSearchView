...
internal fun <T> MaterialSearchView<T>.initSearchBackground(attrs: TypedArray) {
    val materialSearchView: MaterialSearchView<T> = this
    with(attrs) {
        with(cardViewSearch) { layoutParams.height = layoutParams.height - 8.dp }
        val searchBackgroundColor: Int = getColor(
            R.styleable.MaterialSearchView_searchBackgroundColor,
            searchBackgroundColor
        )
        materialSearchView.searchBackgroundColor = searchBackgroundColor

        val searchBackgroundRippleColor: Int = getColor(
        ...
        val searchBackgroundOpenDuration: Long = getInteger(
        ...
        val searchBackgroundAnimation: Int = getInteger(
            R.styleable.MaterialSearchView_searchBackgroundAnimation,
            materialSearchView.searchBackgroundAnimation.value
        )
        materialSearchView.searchBackgroundAnimation = SearchBackgroundAnimation.values()
            .find { anim: SearchBackgroundAnimation -> anim.value == searchBackgroundAnimation }
