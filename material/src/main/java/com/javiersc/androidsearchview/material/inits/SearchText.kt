...
internal fun <T> MaterialSearchView<T>.initSearchText(attrs: TypedArray) {
    ...
    with(attrs) {
        ...
        materialSearchView.searchTextFont =
            if (searchTextFont != 0) materialSearchView.context.font(searchTextFont) else materialSearchView.searchTextFont
        ...
        searchTextView.onFocusChangeListener =
            View.OnFocusChangeListener { _, hasFocus ->
                with(materialSearchView) {
                    if (hasFocus)
                        materialSearchView.setupOpen().also { showSoftKeyboard(context) }
                    else
                        materialSearchView.setupClose().also { hideSoftKeyboard(context) }
                }
            }
    }
}